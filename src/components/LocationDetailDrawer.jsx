...
export default function LocationDetailDrawer({ location, onClose, onNavigate, onPanorama }) {
  ...
  return (
    <div className="fixed inset-0 z-[1500] grid place-items-center overflow-y-auto bg-blue-950/50 p-2 backdrop-blur-sm sm:p-5" onMouseDown={(e) => e.target === e.currentTarget && onClose?.()}>
      <section className="my-auto flex max-h-[96dvh] w-full max-w-5xl flex-col overflow-hidden rounded-[1.5rem] border border-blue-100 bg-white shadow-2xl sm:max-h-[92dvh] sm:rounded-[1.75rem]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain"><LocationDetailContent location={location} onNavigate={onNavigate} onPanorama={onPanorama} /></div>
      </section>
    </div>
  )
}
